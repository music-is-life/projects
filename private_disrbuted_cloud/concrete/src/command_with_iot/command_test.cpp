...
public:
    Client();
    static void OnUpdate(std::shared_ptr<Info> info);

    Callback<std::shared_ptr<Info>> m_callback;
private:
};

void Client::OnUpdate(std::shared_ptr<Info> info)
{
    std::cout << "------------ Notify: " << info->header.m_requestUid << "-----------\n";
}

Client::Client(): m_callback(Function<void(std::shared_ptr<Info>)>(&OnUpdate)) {}
...
int main(void)
{
    ...
    Transmitor::Add(subscriber.m_callback);
